...
public interface HomeworkRepository extends Neo4jRepository<Homework, Long> {
  @Query(
    ...
    "CREATE (homework:Homework {deadline: datetime({epochSeconds: $deadline/1000}), textContent: $textContent})" +
    "<-[has:HAS]-(lesson) " +
  ...
  @Query(
    ...
    "SET homework = {deadline: datetime({epochSeconds: $deadline/1000}), textContent: $textContent} " +
    "RETURN homework"
  )
  ...
  @Query(
    ...
    "SET homework.done = true, homework.handedIn = datetime({epochSeconds: $dateInMillis/1000}), " +
  ...
  @Query(
    ...
    "SET homework.handedIn = datetime({epochSeconds: $dateInMillis/1000}), homework.solution = $solution " +
    "RETURN homework, has, lesson"
  )
